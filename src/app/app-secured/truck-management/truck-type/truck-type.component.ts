...
export class TruckTypeComponent extends BaseComponent implements OnInit {
  txtFilterGb: string;

  //
  parentPage: string = Constant.pages.truckManagementModule.name;
  currentPage: string = Constant.pages.truckManagementModule.childrens.truckType.name;
  //
  ...
  async getData() {
    const data = await this.truckTypeService.getAllAsync();
    if (data) {
  ...
  async save() {
    ...
    if (!(this.dataUpdate.length > 0)) {
      this.messageService.add({
        severity: Constant.messageStatus.warn,
        detail: "Vui lòng nhập chiều dài"
      });
      return;
    }

    if (!(this.dataUpdate.width > 0)) {
      this.messageService.add({
        severity: Constant.messageStatus.warn,
        detail: "Vui lòng nhập chiều rộng"
      });
      return;
    }

    if (!(this.dataUpdate.height > 0)) {
      this.messageService.add({
        severity: Constant.messageStatus.warn,
        detail: "Vui lòng nhập chiều cao"
      });
      return;
    }

    if (this.dataUpdate.id) {
      const res = await this.truckTypeService.updateAsync(this.dataUpdate);
      if (res) {
        this.messageService.add({
      ...
      const res = await this.truckTypeService.createAsync(this.dataUpdate);
      if (res) {
        this.messageService.add({
          severity: Constant.messageStatus.success,
  ...
  async delete() {
    ...
    const data = await this.truckTypeService.deleteAsync(obj);
    if (data) {
      this.messageService.add({
        severity: Constant.messageStatus.success,
